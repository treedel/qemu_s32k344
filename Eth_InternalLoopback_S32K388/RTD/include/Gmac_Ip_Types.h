...
#define GMAC_IP_TYPES_AR_RELEASE_MINOR_VERSION       9
#define GMAC_IP_TYPES_AR_RELEASE_REVISION_VERSION    0
#define GMAC_IP_TYPES_SW_MAJOR_VERSION               7
#define GMAC_IP_TYPES_SW_MINOR_VERSION               0
#define GMAC_IP_TYPES_SW_PATCH_VERSION               1
...
typedef enum
{
    ...
    GMAC_SPEED_2500M = 1U  /*!< Speed 2500 Mbps. */
#endif
} Gmac_Ip_SpeedType;
...
/*!
 * @brief Represents the transmission selection of a queue at the egress side.
 */
typedef enum {
    GMAC_TRANSMISSION_SELECTION_UNSHAPED  = 0U,  /*!< Transmission in an unshaped manner. */
    GMAC_TRANSMISSION_SELECTION_ATS       = 1U,  /*!< Transmission according the asynchronous traffic shaping algorithm. */
    GMAC_TRANSMISSION_SELECTION_CBS       = 2U,  /*!< Transmission according the credit based shaping algorithm. */
    GMAC_TRANSMISSION_SELECTION_ETS       = 3U   /*!< Transmission according the enhanced transmission selection algorithm. */
} Gmac_Ip_TxSelecAlgoType;
...
/*!
 * @brief The Fixed PPS clocks
 * @implements
 */
typedef enum {
    GMAC_PPS_1HZ_DEFAULT               = 0U, /* PPS output is 1 pulse (of width clk_ptp_i) every second */
    GMAC_PPS_1HZ                       = 1U, /* PPS output is 1 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_2HZ                       = 2U, /* PPS output is 2 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_4HZ                       = 3U, /* PPS output is 4 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_8HZ                       = 4U, /* PPS output is 8 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_16HZ                      = 5U, /* PPS output is 16 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_32HZ                      = 6U, /* PPS output is 32 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_64HZ                      = 7U, /* PPS output is 64 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_128HZ                     = 8U, /* PPS output is 128 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_256HZ                     = 9U, /* PPS output is 256 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_512HZ                     = 10U, /* PPS output is 512 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_1024HZ                    = 11U, /* PPS output is 1024 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_2048HZ                    = 12U, /* PPS output is 2048 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_4096HZ                    = 13U, /* PPS output is 4096 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_8192HZ                    = 14U, /* PPS output is 8192 pulse (duty cycle is approximately 50%) every second */
    GMAC_PPS_16384HZ                   = 15U, /* PPS output is 16384 pulse (duty cycle is approximately 50%) every second */
} Gmac_Ip_FixedPPSClockType;
#endif
...
typedef struct sGmac_Ip_FramePreemption
{
    uint16 TxReleaseAdvanceTime;
    uint16 TxHoldAdvanceTime;
    uint8 PreemptionClassic;
    uint8 TxAdditionalFragSize;
    uint8 RxResidueQueue;
} Gmac_Ip_FramePreemption;
#endif
...
{
    uint8 DMAChannel;
    uint8 RxQueueMap[FEATURE_GMAC_NUM_QUEUES];
...
typedef struct sGmac_Ip_TxRingConfigType
{
    ...
    Gmac_Ip_TxSelecAlgoType TxSelecAlgoType;/*!< Selection of a queue at the egress side. */
    ...
    uint16 MTLQueueSize;                    /*!< MTL Egress Queue size. */
...
typedef struct sGmac_Ip_RxRingConfigType
{
    ...
    uint16 BufferLen;                       /*!< Length of each individual buffer in a pool */
    uint16 RingSize;                        /*!< Buffer descriptors number. */
    uint16 MTLQueueSize;                    /*!< MTL Ingress Queue size. */
    uint8 PriorityMask;                     /*!< A logical OR of "Gmac_Ip_VlanPriorityType". */
    uint8 DmaBurstLength;                   /*!< Maximum number of beats in one DMA block data transfer. */
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
...
{
    uint8 PPSIndex;   /*!< Index of corresponding PPS. */
    uint8 PPSDutyCycle;
    uint32 PPSFrequency;
...
/*!
 * @brief Defines the DMA Burst configuration
 */
typedef struct
{
    boolean EnFixedBurstLength;  /*!< When true, DMA initiates transfers of specified length (INCRx or SINGLE).
                                      When false, DMA initiates transfers of unspecified length (INCR) or SINGLE transfers. */
} Gmac_Ip_DmaBurstConfigType;
...
typedef struct sGmac_Ip_ConfigType
{
    ...
    Gmac_Ip_DmaBurstConfigType DmaBurstConfig;
#ifdef GMAC_IP_DMA_PRIORITY_CONFIGURATION_ENABLE
    #if (GMAC_IP_DMA_PRIORITY_CONFIGURATION_ENABLE == STD_ON)
...
typedef struct sGmac_Ip_StateType
{
    ...
    Gmac_Ip_ChCallbackType RxChCallback[FEATURE_GMAC_RX_NUM_CHANNELS];         /*!< Callback function for receive channels. */
    Gmac_Ip_ChCallbackType TxChCallback[FEATURE_GMAC_TX_NUM_CHANNELS];         /*!< Callback function for transmit channels. */
    Gmac_Ip_BufferDescriptorType *TxCurrentDesc[FEATURE_GMAC_TX_NUM_CHANNELS]; /*!< The current available transmit buffer descriptor pointer array. */
    Gmac_Ip_BufferDescriptorType *RxCurrentDesc[FEATURE_GMAC_RX_NUM_CHANNELS]; /*!< The current available receive buffer descriptor pointer array. */
    Gmac_Ip_BufferDescriptorType *RxAllocDesc[FEATURE_GMAC_RX_NUM_CHANNELS];   /*!< Pointer array used for enqueuing Rx buffers provided using Gmac_Ip_ProvideRxBuff. */
    uint16 RxRingCount;
    uint16 TxRingCount;
    ...
    uint16  HeaderSplitOffset[FEATURE_GMAC_RX_NUM_CHANNELS];           /*!< Offset where the payload will be put in the data frame. */
    boolean SplitHeaderSupport[FEATURE_GMAC_RX_NUM_CHANNELS];          /*!< Enable/Disable support for the split header functionality. */
} Gmac_Ip_StateType;
/** @endcond */
