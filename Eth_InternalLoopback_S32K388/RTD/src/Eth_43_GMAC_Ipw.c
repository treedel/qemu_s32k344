...
#define ETH_43_GMAC_IPW_AR_RELEASE_MINOR_VERSION_C       9
#define ETH_43_GMAC_IPW_AR_RELEASE_REVISION_VERSION_C    0
#define ETH_43_GMAC_IPW_SW_MAJOR_VERSION_C               7
#define ETH_43_GMAC_IPW_SW_MINOR_VERSION_C               0
#define ETH_43_GMAC_IPW_SW_PATCH_VERSION_C               1
...
*                                       LOCAL MACROS
==================================================================================================*/
#define ETH_43_GMAC_IPW_TIMEOUT_MS_U16                 ((uint32)(GMAC_TIMEOUT_VALUE_US / 1000UL))
...
#define ETH_43_GMAC_IPW_NANOSECONDS_PER_SECOND       (1000000000U)
...
==================================================================================================*/
    ...
    #define Eth_43_GMAC_Ipw_Local_Macro_TxRxClkMuxBypass(CtrlIdx, TxRxClkMuxBypass)     OsIf_Trusted_Call2params(Eth_43_GMAC_Ipw_TxRxClkMuxBypass, CtrlIdx, (TxRxClkMuxBypass))
#endif
#else
...
#if defined(GMAC_IP_MAC_TXRX_CLK_MUX_BYPASS) && (STD_ON == GMAC_IP_MAC_TXRX_CLK_MUX_BYPASS)
    #define Eth_43_GMAC_Ipw_Local_Macro_TxRxClkMuxBypass(CtrlIdx, TxRxClkMuxBypass)     Eth_43_GMAC_Ipw_TxRxClkMuxBypass(CtrlIdx, TxRxClkMuxBypass)
#endif
#endif
...
* @return        void
*/
void Eth_43_GMAC_Ipw_TxRxClkMuxBypass(uint8 CtrlIdx, const Gmac_Ip_TxRxClkMuxBypassType TxRxClkMuxBypass)
{
    if (CtrlIdx < 1U)
    {
    #if defined(S32K389)
        IP_DCM_GPR->DCMRWF2 = (IP_DCM_GPR->DCMRWF2 & ~DCM_GPR_DCMRWF2_MAC_TX_RX_CLK_MUX_BYPASS_MASK) | (TxRxClkMuxBypass.DCMRWF2);
    #endif
        IP_DCM_GPR->DCMRWF3 = (IP_DCM_GPR->DCMRWF3 & ~(DCM_GPR_DCMRWF3_MAC_TX_CLK_MUX_BYPASS_MASK | DCM_GPR_DCMRWF3_MAC_RX_CLK_MUX_BYPASS_MASK)) | (TxRxClkMuxBypass.DCMRWF3);
    #if (defined(S32K388) || defined(S32K389))
        IP_DCM_GPR->DCMRWF4 = (IP_DCM_GPR->DCMRWF4 & ~DCM_GPR_DCMRWF4_MAC_RMII_CLK_MUX_BYPASS_MASK) | (TxRxClkMuxBypass.DCMRWF4);
    #endif
    }
    else
    {
    #if defined(S32K388)
        IP_DCM_GPR->DCMRWF4 = (IP_DCM_GPR->DCMRWF4 & ~(DCM_GPR_DCMRWF4_MAC2_TX_CLK_MUX_BYPASS_MASK | DCM_GPR_DCMRWF4_MAC2_RX_CLK_MUX_BYPASS_MASK | DCM_GPR_DCMRWF4_MAC2_RMII_CLK_MUX_BYPASS_MASK)) | (TxRxClkMuxBypass.DCMRWF4);
    #elif defined(S32K389)
        IP_DCM_GPR->DCMRWF4 = (IP_DCM_GPR->DCMRWF4 & ~(DCM_GPR_DCMRWF4_MAC2_TX_RX_CLK_MUX_BYPASS_MASK | DCM_GPR_DCMRWF4_MAC2_TX_CLK_MUX_BYPASS_MASK | DCM_GPR_DCMRWF4_MAC2_RX_CLK_MUX_BYPASS_MASK | DCM_GPR_DCMRWF4_MAC2_RMII_CLK_MUX_BYPASS_MASK)) | (TxRxClkMuxBypass.DCMRWF4);
    #endif
    }
}
#endif
...
*
* @param[in]     CtrlIdx      Index of the controller
* @param[in]     QueueIdx     Index of the queue from which the frame was read
* @param[in]     FrameData    Pointer to the frame data
*
* @return        void
*/
void Eth_43_GMAC_Ipw_ReleaseResources(const uint8 CtrlIdx, const uint8 QueueIdx, uint8 * FrameData)
{
    Gmac_Ip_BufferType RxFrameBuffer;

    RxFrameBuffer.Data = FrameData;

    Gmac_Ip_ProvideRxBuff(CtrlIdx, QueueIdx, &RxFrameBuffer);
}
...
Std_ReturnType Eth_43_GMAC_Ipw_GetCurrentTimeTuple(uint8 CtrlIdx, \
                                                    uint8 ClkUnitIdx, \
                                                    ...
                                                    TimeTupleType *pCurrentTimeTuplePtr)
{
    ...
    pCurrentTimeTuplePtr->timestampClockValue.nanoseconds = TimeStamp.nanoseconds;
    pCurrentTimeTuplePtr->timestampClockValue.seconds = TimeStamp.seconds;
    pCurrentTimeTuplePtr->timestampClockValue.secondsHi = TimeStamp.secondsHi;

    /* disciplinedClockValue has the same value as timestampClockValue for platforms have only one clock used for timestamping and used as Phc */
    pCurrentTimeTuplePtr->disciplinedClockValue.nanoseconds = pCurrentTimeTuplePtr->timestampClockValue.nanoseconds;
    pCurrentTimeTuplePtr->disciplinedClockValue.seconds = pCurrentTimeTuplePtr->timestampClockValue.seconds;
    pCurrentTimeTuplePtr->disciplinedClockValue.secondsHi = pCurrentTimeTuplePtr->timestampClockValue.secondsHi;
    pCurrentTimeTuplePtr->timeQuality = VALID;
...
Std_ReturnType Eth_43_GMAC_Ipw_GetPhcTime(uint8 CtrlIdx, \
                                            uint8 ClkUnitIdx, \
                                            const Eth_43_GMAC_CtrlCfgType *CtrlConfig, \
                                            ...
                                            TimeStampType *pTimeStampPtr)
{
    ...
    pTimeStampPtr->nanoseconds = TimeStamp.nanoseconds;
    pTimeStampPtr->seconds = TimeStamp.seconds;
    pTimeStampPtr->secondsHi = TimeStamp.secondsHi;
    *pTimeQualPtr = VALID;
...
Std_ReturnType Eth_43_GMAC_Ipw_SetPhcTime(uint8 CtrlIdx, \
                                            uint8 ClkUnitIdx, \
                                            const Eth_43_GMAC_CtrlCfgType *CtrlConfig, \
                                            const TimeStampType *pTimeStampPtr)
{
    Gmac_Ip_SysTimeConfigType SysTimeConfig = { (uint8)0U, (uint8)0U, (float64)0, NULL_PTR };
    Gmac_Ip_TimestampType SysTime;
    const Eth_43_GMAC_ClockCfgType *ClkCfg = CtrlConfig->Eth_43_GMAC_pClockConfig;

    (void)ClkUnitIdx;

    SysTime.nanoseconds = pTimeStampPtr->nanoseconds;
    SysTime.seconds = pTimeStampPtr->seconds;
    SysTime.secondsHi = pTimeStampPtr->secondsHi;
...
Std_ReturnType Eth_43_GMAC_Ipw_SetPhcCorrection(uint8 CtrlIdx, \
                                                    uint8 ClkUnitIdx, \
                                                    const Eth_43_GMAC_CtrlCfgType *CtrlConfig, \
                                                    ...
                                                    sint32 offset)
{
    ...
    /* Get abs value of offset */
    Nanosecond = (offset >= 0) ? (uint32)offset : (uint32)((-1) * offset);

    Status = Gmac_Ip_SetSysTimeCorr(CtrlIdx, \
                                    ((offset > 0) ? (Gmac_Ip_SysTimeCorrOffsetType)GMAC_SYS_TIME_CORR_POSITIVE : (Gmac_Ip_SysTimeCorrOffsetType)GMAC_SYS_TIME_CORR_NEGATIVE), \
                                    Nanosecond / ETH_43_GMAC_IPW_NANOSECONDS_PER_SECOND, \
                                    Nanosecond % ETH_43_GMAC_IPW_NANOSECONDS_PER_SECOND);
    ...
    if (GMAC_STATUS_SUCCESS == Status)
    {
        ...
        if (rateDeviation > 0)
        {
            FractionalPortion = (float64)rateDeviation / (float64)(0x20000000000UL); /* Rate deviation resolution: 2^(-41) */
            AddendTimeUpdate = ((float64)1.0 - FractionalPortion) * AddendTime; /* speed up counter */

        }
        else if (rateDeviation < 0)
        {
            FractionalPortion = (float64)((-1) * rateDeviation) / (float64)(0x20000000000UL); /* Rate deviation resolution: 2^(-41) */
            AddendTimeUpdate = ((float64)1.0 + FractionalPortion) * AddendTime; /* slow down counter */
        }
        else
        {
            /* back to the original rate */
            AddendTimeUpdate = AddendTime;
        }
        Status = Gmac_Ip_SetRateRatioCorr(CtrlIdx, AddendTimeUpdate);
    }
...
#if defined(GMAC_IP_MAC_TXRX_CLK_MUX_BYPASS) && (STD_ON == GMAC_IP_MAC_TXRX_CLK_MUX_BYPASS)
    Eth_43_GMAC_Ipw_Local_Macro_TxRxClkMuxBypass(CtrlIdx, IpwCtrlCfg->Eth_43_GMAC_Ipw_TxRxClkMuxBypassConfig);
#endif
...
* @param[in]    FifoIdx         The fifo index
* @param[in]    Frame           A structure defining the multi buffer frame
* @param[in]    BufIdxPtr       Index to the granted transmit buffer resource. It uniquely identifies the buffer in all subsequent calls of Eth_TxConfirmation().
                                                               ...
                                                               boolean TxConfirmation)
{
    ...
    status = Gmac_Ip_GetTxMultiBuff(CtrlIdx, FifoIdx, Frame.NumBuffers, Frame.BufferLength, &u16BufferIdxWithinFifo);

    if (GMAC_STATUS_SUCCESS == status)
    {
        /* Generate a unique buffer id */
        *BufIdxPtr = Eth_43_GMAC_Ipw_axFirstTxBufferIdxInFifo[CtrlIdx][FifoIdx] + u16BufferIdxWithinFifo;
        BuffIdx = (*BufIdxPtr);

        for (FrameIndex = 0U; FrameIndex < Frame.NumBuffers; FrameIndex++)
        {
            ...
            Eth_43_GMAC_Ipw_MultiBufferFrame[FrameIndex].Data = Frame.BufferData[FrameIndex];
            Eth_43_GMAC_Ipw_MultiBufferFrame[FrameIndex].Length = Frame.BufferLength[FrameIndex];
            ...
            SchM_Enter_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_03();
            ...
            SchM_Exit_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_03();

            BuffIdx = ((BuffIdx + 1U) >= (Eth_43_GMAC_Ipw_axFirstTxBufferIdxInFifo[CtrlIdx][FifoIdx] + (uint32)((Eth_43_GMAC_Ipw_apxInternalCfg[CtrlIdx]->Eth_43_GMAC_Ipw_pGmacCtrlConfig->Gmac_paCtrlTxRingConfig)[FifoIdx].RingSize)))? \
                      (Eth_43_GMAC_Ipw_axFirstTxBufferIdxInFifo[CtrlIdx][FifoIdx]) : (BuffIdx + 1U);
        }

        status = Gmac_Ip_SendMultiBufferFrame(CtrlIdx, FifoIdx, Eth_43_GMAC_Ipw_MultiBufferFrame, &TxOptions, Frame.NumBuffers);
    }
                            ...
                            )
{
    ...
    return ((GMAC_STATUS_SUCCESS == Gmac_Ip_MDIOWrite(CtrlIdx, Trcv, Reg, Data, ETH_43_GMAC_IPW_TIMEOUT_MS_U16))? (TRUE) : (FALSE));
}
...
{
    /* 1 millisecond is enough to transfer about 16 MDIO frames at an MDC frequency of 1MHz. */
    return ((GMAC_STATUS_SUCCESS == Gmac_Ip_MDIORead(CtrlIdx, Trcv, Reg, Data, ETH_43_GMAC_IPW_TIMEOUT_MS_U16))? (TRUE) : (FALSE));
}
#endif
                        ...
                        )
{
    ...
    return ((GMAC_STATUS_SUCCESS == Gmac_Ip_MDIOReadMMD(CtrlIdx, TrcvIdx, Mmd, RegAddress, RegValPtr, ETH_43_GMAC_IPW_TIMEOUT_MS_U16))? (TRUE) : (FALSE));
}
...
{
    /* 1 millisecond is enough to transfer about 8 MMD frames at an MDC frequency of 1MHz. */
    return ((GMAC_STATUS_SUCCESS == Gmac_Ip_MDIOWriteMMD(CtrlIdx, TrcvIdx, Mmd, RegAddress, RegVal, ETH_43_GMAC_IPW_TIMEOUT_MS_U16))? (TRUE) : (FALSE));
}
#endif
                           ...
                           )
{
    ...
    {
        SchM_Enter_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_02();
        ...
        while ((Iter != NULL_PTR))
        {
            ...
            if (GMAC_STATUS_SUCCESS == GmacStatus)
            {
                ...
                for (NumberOfClearedBd = 0U; NumberOfClearedBd < Gmac_Ip_u16NumberOfClearedBd[CtrlIdx][FifoIdx]; NumberOfClearedBd++)
                {
            ...
            Gmac_Ip_u16NumberOfClearedBd[CtrlIdx][FifoIdx] = 0U;
        }
        SchM_Exit_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_02();
...
{
    Gmac_Ip_FixedPPSClockType FixedClock;
    ...
    if (SignalMode)
    {
        /* Mapping integer to enum */
        switch(CtrlConfig->Eth_43_GMAC_pIpwCtrlConfig->PPSFrequency)
        {
            case 1U:
            {
                FixedClock = GMAC_PPS_1HZ;
                break;
            }
            case 2U:
            {
                FixedClock = GMAC_PPS_2HZ;
                break;
            }
            case 4U:
            {
                FixedClock = GMAC_PPS_4HZ;
                break;
            }
            case 8U:
            {
                FixedClock = GMAC_PPS_8HZ;
                break;
            }
            case 16U:
            {
                FixedClock = GMAC_PPS_16HZ;
                break;
            }
            case 32U:
            {
                FixedClock = GMAC_PPS_32HZ;
                break;
            }
            case 64U:
            {
                FixedClock = GMAC_PPS_64HZ;
                break;
            }
            default:
            {
                FixedClock = GMAC_PPS_1HZ_DEFAULT;
                break;
            }
        }
        /* switch to Fix PPS mode to generate fixed pulses */
        Gmac_Ip_SetPpsSignalMode(CtrlIdx, FALSE);
        Gmac_Ip_GenerateFixedPulsePerSecondOutput(CtrlIdx, FixedClock);
    }
    else
    {
        Gmac_Ip_SetPpsSignalMode(CtrlIdx, TRUE);
    }
...
{
    uint8 PPSCnt = 0U;
    Gmac_Ip_FlexiblePPSOutput FlexiblePPSOutput[FEATURE_GMAC_PPS_NUM_CHANNELS];

    for (PPSCnt = 0; PPSCnt < PPSOutputsNum; PPSCnt++)
    {
        FlexiblePPSOutput[PPSCnt].PPSIndex = PPSOutputConfig[PPSCnt].PPSIndex;
        FlexiblePPSOutput[PPSCnt].PPSDutyCycle = PPSOutputConfig[PPSCnt].PPSDutyCycle;
        FlexiblePPSOutput[PPSCnt].PPSFrequency = PPSOutputConfig[PPSCnt].PPSFrequency;
        FlexiblePPSOutput[PPSCnt].PPSModeSelect = GMAC_GENERATE_ONLY_PPS_START_STOP_TIME;
    }
    Gmac_Ip_PPSOutputSignalInit(CtrlIdx, CtrlConfig->Eth_43_GMAC_pClockConfig->ModuleClock, PPSOutputsNum, FlexiblePPSOutput);
