...
#define GMAC_IP_HW_ACCESS_AR_RELEASE_MINOR_VERSION_C       9
#define GMAC_IP_HW_ACCESS_AR_RELEASE_REVISION_VERSION_C    0
#define GMAC_IP_HW_ACCESS_SW_MAJOR_VERSION_C               7
#define GMAC_IP_HW_ACCESS_SW_MINOR_VERSION_C               0
#define GMAC_IP_HW_ACCESS_SW_PATCH_VERSION_C               1
...
/* Checks against Gmac_Ip_Hw_Access.h */
    ...
    #if (( GMAC_IP_HW_ACCESS_AR_RELEASE_MAJOR_VERSION_C    !=  MCAL_AR_RELEASE_MAJOR_VERSION) || \
         ( GMAC_IP_HW_ACCESS_AR_RELEASE_MINOR_VERSION_C    !=  MCAL_AR_RELEASE_MINOR_VERSION)    \
        )
        #error "AUTOSAR Version Numbers of Gmac_Ip_Hw_Access.c and Mcal.h are different"
    #endif
#endif
                              ...
                              )
{
    ...
    {
        /* If the driver is not initialized, clear only the interrupt status flag for the enabled interrupts. */
        if ((ErrorEnFlags & GMAC_ECC_CTL_MASK) != 0U)
        {
            /* Clear the ECC interrupt event(s) */
            Base->MTL_ECC_INTERRUPT_STATUS = ErrorStatusFlags;
        }
...
static void GMAC_MACIRQHandler(uint8 Instance)
{
    ...
    if ((Base->DMA_INTERRUPT_STATUS & GMAC_DMA_INTERRUPT_STATUS_MACIS_MASK) != 0U)
    {
        ...
        {
            IrqEnFlagMasks = Base->MAC_INTERRUPT_ENABLE;
            /* Clear the interrupt status flag (by reading the bits) only for the enabled interrupts when the driver is not initialized. */
            Flags = Base->MAC_INTERRUPT_STATUS & IrqEnFlagMasks;
            (void)Flags;

        #if FEATURE_GMAC_RGMII_EN || FEATURE_GMAC_SMII_EN
            if ((IrqEnFlagMasks & GMAC_MAC_INTERRUPT_ENABLE_RGSMIIIE_MASK) != 0U)
            {
                (void)Base->MAC_PHYIF_CONTROL_STATUS;
            }
        #endif
            if ((IrqEnFlagMasks & GMAC_MAC_INTERRUPT_ENABLE_FPEIE_MASK) != 0U)
            {
                (void)Base->MAC_FPE_CTRL_STS;
            }
            if (((IrqEnFlagMasks & GMAC_MAC_INTERRUPT_ENABLE_RXSTSIE_MASK) != 0U) ||
                ((IrqEnFlagMasks & GMAC_MAC_INTERRUPT_ENABLE_TXSTSIE_MASK) != 0U))
            {
                (void)Base->MAC_RX_TX_STATUS;
            }
        }
 ...
 * interrupt flags.
 * implements     GMAC_CommonIRQHandler_Activity
 *END**************************************************************************/
void GMAC_CommonIRQHandler(uint8 Instance)
{
    ...
    if (Gmac_apxState[Instance] != NULL_PTR)
    {
        for (i = 0U; i < FEATURE_GMAC_NUM_CHANNELS; i++)
        {
            ...
            {
                ChBase = Gmac_apxChBases[Instance][i];
    ...
    else
    {
        for (i = 0U; i < FEATURE_GMAC_NUM_CHANNELS; i++)
        {
            if ((Base->DMA_INTERRUPT_STATUS & (1UL << (uint32)i)) != 0U)
            {
                ChBase = Gmac_apxChBases[Instance][i];
                Flags = ChBase->DMA_STATUS;
                IrqEnFlagMasks = ChBase->DMA_INTERRUPT_ENABLE;
                /* Clear the interrupt status flag (W1C) only for the enabled interrupts when the driver is not initialized. */
                ChBase->DMA_STATUS = Flags & IrqEnFlagMasks;
                (void)ChBase->DMA_STATUS;
            }
        }
    }
 ...
 * This handler invokes the installed callback (if available) and clears the
 * interrupt flags.
 * implements     GMAC_SafetyIRQHandler_Activity
 *END**************************************************************************/
                              ...
                              uint8 QueuesNum)
{
    ...
    if (QueuesNum > 5U)
    {
        Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q5DDMACH(0U) |
                                  GMAC_MTL_RXQ_DMA_MAP1_Q5MDMACH(5U);
    }
#endif
#if FEATURE_GMAC_RX_NUM_CHANNELS > 6U
    if (QueuesNum > 6U)
    {
        Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q6DDMACH(0U) |
                                  GMAC_MTL_RXQ_DMA_MAP1_Q6MDMACH(6U);
    }
#endif
#if FEATURE_GMAC_RX_NUM_CHANNELS > 7U
    if (QueuesNum > 7U)
    {
        Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q7DDMACH(0U) |
                                  GMAC_MTL_RXQ_DMA_MAP1_Q7MDMACH(7U);
    }
#endif
}
