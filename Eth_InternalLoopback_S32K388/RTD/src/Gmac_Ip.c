...
#define GMAC_IP_AR_RELEASE_MINOR_VERSION_C       9
#define GMAC_IP_AR_RELEASE_REVISION_VERSION_C    0
#define GMAC_IP_SW_MAJOR_VERSION_C               7
#define GMAC_IP_SW_MINOR_VERSION_C               0
#define GMAC_IP_SW_PATCH_VERSION_C               1
...
/* Checks against Gmac_Ip_TrustedFunctions.h */
    ...
    #if (( GMAC_IP_AR_RELEASE_MAJOR_VERSION_C    !=  MCAL_AR_RELEASE_MAJOR_VERSION) || \
         ( GMAC_IP_AR_RELEASE_MINOR_VERSION_C    !=  MCAL_AR_RELEASE_MINOR_VERSION)    \
        )
        #error "AUTOSAR Version Numbers of Gmac_Ip.c and Mcal.h are different"
    #endif
#endif
...
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
...
#define GMAC_RDES2_HL_MASK      (0x000003FFU)
...
#define GMAC_IPV6_GET_HIGHER_BYTE(x)              ((uint32)(x) & 0xFFU)
#define GMAC_IPV6_GET_LOWER_BYTE(x)               (((uint32)(x) & 0xFF00U) >> 8U)
#define GMAC_IPV6_LOWER_BITS_MASK                 (0x1FU)
#define GMAC_IPV6_HIGHER_BITS_MASK                (0x60U)

#define GMAC_MAC_L3_L4_CONTROL_REG_OFFSET(x)      ((uint32)(x) * 0x30U)
#endif
...
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
static const uint16 Gmac_Ip_ScatterGatherLastTxBuffLength[FEATURE_GMAC_NUM_INSTANCES] = GMAC_IP_SCATTER_GATHER_TX_MAX_TXBUFFLEN;
#endif
...
*                                        GLOBAL VARIABLES
==================================================================================================*/
#define ETH_43_GMAC_START_SEC_VAR_INIT_16
#include "Eth_43_GMAC_MemMap.h"

volatile uint16 Gmac_Ip_u16NumberOfClearedBd[FEATURE_GMAC_NUM_INSTANCES][FEATURE_GMAC_NUM_CHANNELS] = {0U};

#define ETH_43_GMAC_STOP_SEC_VAR_INIT_16
#include "Eth_43_GMAC_MemMap.h"
/*==================================================================================================
...
#if (STD_ON == GMAC_IP_HAS_RX_L3_L4_FILTERS)
static void Gmac_Ip_SetRxQueuesDmaChDynamicMap(GMAC_Type * Base,
                                               uint8 QueuesNum);
...
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
static void Gmac_Ip_CopyData(const uint8 * Src,  uint8 *Dest, uint16 Length, uint16 Offset);
#endif

#if (STD_ON == GMAC_IP_FRAME_PREEMPTION_ENABLE)
static inline void Gmac_Ip_FramePreemptionInit(GMAC_Type *Base, const Gmac_Ip_FramePreemption *FramePreemptionConfig);
#endif

static Gmac_Ip_StatusType Gmac_Ip_ProcessTxDesc(uint8 Instance, uint8 Ring, Gmac_Ip_BufferDescriptorType * BdTemp, Gmac_Ip_TxInfoType * Info);
static Gmac_Ip_StatusType Gmac_Ip_ProcessRxDesc(uint8 Instance,
                                                uint8 Ring,
                                                Gmac_Ip_BufferDescriptorType * BdTemp,
                                          ...
                                          const Gmac_CtrlConfigType *Config)
{
    ...
    if (Status == GMAC_STATUS_SUCCESS)
    {
        ...
        if (TRUE == Config->Gmac_pCtrlConfig->DmaBurstConfig.EnFixedBurstLength)
        {
            Base->DMA_SYSBUS_MODE |= GMAC_DMA_SYSBUS_MODE_FB(1U);
        }
                            ...
                            const Gmac_CtrlConfigType *Config)
{
    ...
    {
        FifoSize = (uint32)Config->Gmac_paCtrlTxRingConfig[i].MTLQueueSize;
        ...
        else if ((GMAC_OP_MODE_AVB == Config->Gmac_paCtrlTxRingConfig[i].QueueOpMode) && (i > 0U))
        {
            ...
            if (Config->Gmac_paCtrlTxRingConfig[i].TxSelecAlgoType == GMAC_TRANSMISSION_SELECTION_CBS)
            {
    ...
    {
        FifoSize = (uint32)Config->Gmac_paCtrlRxRingConfig[i].MTLQueueSize;
                            ...
                            const Gmac_CtrlConfigType *Config)
{
...
#if (STD_ON == GMAC_IP_COMBINED_IRQ)
    /* Clear the interrupt flags before enabling the interrupts. */
    if (((Base->DMA_INTERRUPT_STATUS & GMAC_DMA_INTERRUPT_STATUS_MACIS_MASK) != 0U) && ((Gmac_apxPcieBases->ETH_INT_MASK & QTM_PCIE_ETH_CREG_ETH_INT_MASK_COMBINED_IRQ_MASK) != 0U))
    {
#else
    /* Clear the interrupt flags before enabling the interrupts. */
    ...
    #if (STD_ON == GMAC_IP_COMBINED_IRQ)
        Gmac_apxPcieBases->ETH_INT_MASK_CLR |= QTM_PCIE_ETH_CREG_ETH_INT_MASK_CLR_COMBINED_IRQ_MASK;
    #endif
...
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
    /* Clear type of split and the offset. */
    ...
    for (i = 0U; i < Config->RxRingCount; i++)
    {
        for (j = 0U; j < RxRingConfig[i].RingSize; j++)
        {
            if (RxRingConfig[i].Buffer != NULL_PTR)
            {
                ...
                RxRingConfig[i].RingDesc[j].Padding0 = (uint32)(Gmac_Ip_PtrSizeType)(&RxRingConfig[i].Buffer[RxRingConfig[i].BufferLen * j]);
                ...
                if (TRUE == RxRingConfig[i].SplitHeaderSupport)
                {
                    ...
                    /* Store the address of Buffer2 or Payload in Padding1 for further uses. */
                    RxRingConfig[i].RingDesc[j].Padding1 = RxRingConfig[i].RingDesc[j].Des2;
                }
                else
                {
                    RxRingConfig[i].RingDesc[j].Des2  = 0U;
                    RxRingConfig[i].RingDesc[j].Padding1 = 0U;
                }
#else
                RxRingConfig[i].RingDesc[j].Des2  = 0U;
#endif
        ...
        *ControlFilterRegAddr |= RxL3L4FilterConfig[FilterNum].MacL3L4FilterEnable;
        ...
        if ((*ControlFilterRegAddr & GMAC_MAC_L3_L4_CONTROL0_DMCHEN0_MASK) != 0U)
        {
            *ControlFilterRegAddr |= GMAC_MAC_L3_L4_CONTROL0_DMCHN0(RxL3L4FilterConfig[FilterNum].DMAChannel);
            /* Dynamic mapping between DMA channel number programmed in the L3-L4 filter registers and the selected receive queues. */
            for (QueueNum = 0U; QueueNum < FEATURE_GMAC_NUM_QUEUES; QueueNum++)
            {
                if (RxL3L4FilterConfig[FilterNum].RxQueueMap[QueueNum] != 0U)
                {
                    Gmac_Ip_SetRxQueuesDmaChDynamicMap(Base, QueueNum);
                }
            }
        }
        ...
        if ((*ControlFilterRegAddr & GMAC_MAC_L3_L4_CONTROL0_L3PEN0_MASK) == 0U) /* IPv4 Packet */
        {
            *ControlFilterRegAddr |= GMAC_MAC_L3_L4_CONTROL0_L3HSBM0(RxL3L4FilterConfig[FilterNum].Layer3IPv4SourceBitsMatch) |
            ...
            {
                Layer3RegAddr[0U] |= ((uint32)RxL3L4FilterConfig[FilterNum].Layer3IPv4SourceAddress[0U] << GMAC_BYTE_3_IN_ADDR_SHIFT) |
        ...
        {
            *ControlFilterRegAddr |= GMAC_MAC_L3_L4_CONTROL0_L3HSBM0((uint32)(RxL3L4FilterConfig[FilterNum].Layer3IPv6BitsMatch) & GMAC_IPV6_LOWER_BITS_MASK) |
                                     GMAC_MAC_L3_L4_CONTROL0_L3HDBM0(((uint32)(RxL3L4FilterConfig[FilterNum].Layer3IPv6BitsMatch) & GMAC_IPV6_HIGHER_BITS_MASK) >> 5U);

            if ((*ControlFilterRegAddr & GMAC_MAC_L3_L4_CONTROL0_L3SAM0_MASK) != 0U)
            {
                Layer3RegAddr[0U] |= (GMAC_IPV6_GET_LOWER_BYTE(RxL3L4FilterConfig[FilterNum].Layer3IPv6SourceAddress[0U]) << GMAC_BYTE_3_IN_ADDR_SHIFT) |
            ...
            {
                Layer3RegAddr[0U] |= (GMAC_IPV6_GET_LOWER_BYTE(RxL3L4FilterConfig[FilterNum].Layer3IPv6DestinationAddress[0U]) << GMAC_BYTE_3_IN_ADDR_SHIFT) |
...
static boolean Gmac_Ip_RestoreRxCtxtDescr(Gmac_Ip_BufferDescriptorType *Bd)
{
    ...
    if (Bd->Info0 != 0U)
    {
        ...
        Bd->Des1  = 0U;
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
        /* Restore the address of the payload when the split header feature is enabled. The header address is stored in Info0. */
        Bd->Des2  = Bd->Padding1;
#else
        Bd->Des2  = 0U;
#endif
...
static void Gmac_Ip_RestoreRxDescr(uint8 Instance)
{
    ...
    {
        /* Store the position at which the Rx process was previously stopped. */
        Gmac_apxState[Instance]->RxCurrentDesc[i] = (Gmac_Ip_BufferDescriptorType *)((Gmac_Ip_PtrSizeType)Gmac_apxChBases[Instance][i]->DMA_CURRENT_APP_RXDESC);
        Gmac_apxState[Instance]->RxAllocDesc[i] = (Gmac_Ip_BufferDescriptorType *)((Gmac_Ip_PtrSizeType)Gmac_apxChBases[Instance][i]->DMA_CURRENT_APP_RXDESC);
...
static inline void Gmac_Ip_FramePreemptionInit(GMAC_Type *Base, const Gmac_Ip_FramePreemption *FramePreemptionConfig)
{
    ...
    Base->MAC_FPE_CTRL_STS |= GMAC_MAC_FPE_CTRL_STS_EFPE_MASK;

    /* The TX Queues whose packets are preemptable are indicated by setting the PEC field. The earliest starting position of preemption is controlled by the AFSZ field. */
    /* Note: The first Queue (Q0) is always programmed to carry preemption traffic and therefore it is always Open. The GCL bit of the corresponding Queue are ignored. So, even if the software writes a "0" ("C"), it is ignored for such queues */
    Base->MTL_FPE_CTRL_STS = GMAC_MTL_FPE_CTRL_STS_PEC(FramePreemptionConfig->PreemptionClassic) | GMAC_MTL_FPE_CTRL_STS_AFSZ(FramePreemptionConfig->TxAdditionalFragSize);

    /* The Hold Advance (HADV) time marks the cease of the preemptable traffic. Respectively, the Release Advance (RADV) time marks the resuming of the preemptable traffic. */
    Base->MTL_FPE_ADVANCE = GMAC_MTL_FPE_ADVANCE_HADV(FramePreemptionConfig->TxHoldAdvanceTime) | GMAC_MTL_FPE_ADVANCE_RADV(FramePreemptionConfig->TxReleaseAdvanceTime);

    /* Frame Preemption Residue Queue (FPRQ) is used to route the received preemption packets, in addition to the PSRQ (Priority Selected in Rx queue) based queue routing. */
    Base->MAC_RXQ_CTRL1 |= GMAC_MAC_RXQ_CTRL1_FPRQ(FramePreemptionConfig->RxResidueQueue);
}
#endif
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_CopyData
 * Description   : Copies a number of bytes (Length) from the Src address to the Dest address
 *
 *END**************************************************************************/
static void Gmac_Ip_CopyData(const uint8 * Src,  uint8 *Dest, uint16 Length, uint16 Offset)
{
    uint16 TempVar = Length;

    /* Start copy data*/
    while (TempVar > 0U)
    {
        TempVar--;
        Dest[TempVar + Offset] = Src[TempVar];
    }
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_ProcessTxDesc
 * Description   : Get information and restore the Descriptor
 * This sub-function is called only at Gmac_Ip_GetTransmitStatus
 *END**************************************************************************/
static Gmac_Ip_StatusType Gmac_Ip_ProcessTxDesc(uint8 Instance, uint8 Ring, Gmac_Ip_BufferDescriptorType * BdTemp, Gmac_Ip_TxInfoType * Info)
{
    Gmac_Ip_BufferDescriptorType *ListBd;
    ...
    StartBd = (const Gmac_Ip_BufferDescriptorType *)Bd;   /* Prevent endless loop */

    while (((Bd->Des3 & GMAC_TDES3_LD_MASK) == 0U) && ((Bd->Des3 & GMAC_TDES3_OWN_MASK) == 0U))
    {
        /* Count the number of released buffers for the upper layer to point to the first free buffer. */
        Gmac_Ip_u16NumberOfClearedBd[Instance][Ring]++;
...
# if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    ...
    if (((Bd->Des3 & GMAC_TDES3_LD_MASK) != 0U) && ((Bd->Des3 & GMAC_TDES3_OWN_MASK) == 0U))
    {
        ...
        /* Increase number of released buffers for the upper layer to point to the first free buffer. */
        Gmac_Ip_u16NumberOfClearedBd[Instance][Ring]++;
        ...
        Bd->Info1 &= ~GMAC_INFO1_LOCKED_MASK;
# if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
        /* Clean the fields of the DES3 to ensure a clear descriptor is used for every new transfer */
        Bd->Des3 = 0U;
#endif
    }
    else
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_ProcessRxDesc
 * Description   : Get information and restore the Descriptor
 * This sub-function is called only at Gmac_Ip_ReadFrame
 *END**************************************************************************/
static Gmac_Ip_StatusType Gmac_Ip_ProcessRxDesc(uint8 Instance,
                                                uint8 Ring,
                                                Gmac_Ip_BufferDescriptorType * BdTemp,
                                               ...
                                               )
{
    ...
    Gmac_Ip_BufferDescriptorType * Bd;
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    uint8 CountOfBdUntilQueueEnd = 0U;
    boolean EndOfQueueReached = FALSE;
    uint16 LengthToCopy = 0U;
    uint16 DescriptorDataBuffLen = 0U;
    const uint8* DataBufferSrc;
    uint8* DataBufferDest;
    const Gmac_Ip_BufferDescriptorType *LastBd;
#endif
    ...
    ListBd = (Gmac_Ip_BufferDescriptorType *)((Gmac_Ip_PtrSizeType)ChBase->DMA_RXDESC_LIST_ADDRESS);
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    /* Get the address of the Last Descriptor of the queue */
    LastBd = (const Gmac_Ip_BufferDescriptorType *)&ListBd[ChBase->DMA_RXDESC_RING_LENGTH];
#endif

#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    /* This condition checks if there are multiple descriptors in which a large packet has been splitted */
    if (((Bd->Des3 & GMAC_RDES3_FD_MASK) != 0U))
    {
        /* Loop through the Descriptors to ensure all the packet is received. (search for LD bit) */
        while ((Bd->Des3 & GMAC_RDES3_LD_MASK) == (uint32)0U)
        {
            /* Go to the next descriptor */
            Bd++;
            /* Classic check for reaching the end of the queue. Wrap around the Bd variable. */
            if ((Gmac_Ip_PtrSizeType)Bd >=
                (Gmac_Ip_PtrSizeType)&ListBd[ChBase->DMA_RXDESC_RING_LENGTH + 1UL])
            {
                Bd = ListBd;
            }
            /* Checks if there was no LD bit encontered and the pointer reached the start descriptor - hardware problem with memory read - safety check*/
            if ((Bd == Gmac_apxState[Instance]->RxCurrentDesc[Ring]) && (CountOfBdUntilQueueEnd > 0U))
            {
            ...
            CountOfBdUntilQueueEnd++;
        }

        if (GMAC_STATUS_SUCCESS == Status)
        {
            /* Get the first descriptor address */
            Bd = Gmac_apxState[Instance]->RxCurrentDesc[Ring];
#endif   /* (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)*/

            Buff[0U].Data = (uint8 *)((Gmac_Ip_PtrSizeType)Bd->Info0);
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
            /* Store the address of the payload to another buffer. */
            Buff[1U].Data = (uint8 *)((Gmac_Ip_PtrSizeType)Bd->Padding1);
#endif

#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
            /* Clean up the descriptors used in the reception */
            CountOfBdUntilQueueEnd = 0U;
            while ((Bd->Des3 & GMAC_RDES3_LD_MASK) == (uint32)0U)
            {
                /* Count the descriptors used until the last descriptor in the queue is reached */
                if (FALSE == EndOfQueueReached)
                {
                    CountOfBdUntilQueueEnd++;
                }
            ...
            /* Set the CurrentDesc to the Last Descriptor used in Reception. It will go to the next available descriptor in the next line */
            Gmac_apxState[Instance]->RxCurrentDesc[Ring] = Bd;
#endif /* STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE) */

#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
            Buff[0U].Length = (uint16)(Bd->Des2 & GMAC_RDES2_HL_MASK);
            Buff[1U].Length = (uint16)((Bd->Des3 & GMAC_RDES3_PL_MASK) - (Bd->Des2 & GMAC_RDES2_HL_MASK));
#else
...
#endif
#if (STD_OFF == GMAC_IP_SCATTER_GATHER_ENABLE)
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
            /* According to requirement CPR_RTD_00284.eth */
            if ((Buff[0U].Length + Buff[1U].Length) > (uint16)(Bd->Info1 & GMAC_INFO1_LENGTH_MASK))
            {
                Buff[1U].Length = (uint16)((Bd->Info1 & GMAC_INFO1_LENGTH_MASK) - (Buff[0U].Length));
            }
#else
            /* According to requirement CPR_RTD_00284.eth */
...
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
                Info->PktLen = Buff[0U].Length + Buff[1U].Length;
#else
            ...
            /* must always process the context descriptor if present! */
...
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
            /* In case the received packet is split in multiple descriptors that wrapped around,
            the data must be copied in the last data buffer corresponding to the last descriptor.*/
            if (TRUE == EndOfQueueReached)
            {
                DescriptorDataBuffLen = (uint16)(ListBd->Info1 & GMAC_INFO1_LENGTH_MASK);
                LengthToCopy = (Buff->Length > (CountOfBdUntilQueueEnd * DescriptorDataBuffLen)) ? (uint16)(Buff->Length - (CountOfBdUntilQueueEnd * DescriptorDataBuffLen)) : 0U;
                DataBufferSrc = (const uint8 *)(Gmac_Ip_PtrSizeType)ListBd->Padding0;
                DataBufferDest = (uint8 *)(Gmac_Ip_PtrSizeType)LastBd->Padding0;

                Gmac_Ip_CopyData(DataBufferSrc, DataBufferDest, LengthToCopy, DescriptorDataBuffLen);
            }
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_SetRxQueuesDmaChDynamicMap
 * Description   : Sets the dynamic DMA channel mapping with Rx queues
 *
 *END**************************************************************************/
static void Gmac_Ip_SetRxQueuesDmaChDynamicMap(GMAC_Type * Base,
                                               uint8 QueuesNum)
{
    switch (QueuesNum)
    {
        case 0U:
        {
            Base->MTL_RXQ_DMA_MAP0 |= GMAC_MTL_RXQ_DMA_MAP0_Q0MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP0_Q0DDMACH(1U);
            break;
        }
    #if FEATURE_GMAC_RX_NUM_CHANNELS > 1U
        case 1U:
        {
            Base->MTL_RXQ_DMA_MAP0 |= GMAC_MTL_RXQ_DMA_MAP0_Q1MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP0_Q1DDMACH(1U);
            break;
        }
    #endif
        ...
        case 2U:
        {
            Base->MTL_RXQ_DMA_MAP0 |= GMAC_MTL_RXQ_DMA_MAP0_Q2MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP0_Q2DDMACH(1U);
            break;
        }
    #endif
    #if FEATURE_GMAC_RX_NUM_CHANNELS > 3U
        case 3U:
        {
            Base->MTL_RXQ_DMA_MAP0 |= GMAC_MTL_RXQ_DMA_MAP0_Q3MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP0_Q3DDMACH(1U);
            break;
        }
    #endif
    #if FEATURE_GMAC_RX_NUM_CHANNELS > 4U
        case 4U:
        {
            Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q4MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP1_Q4DDMACH(1U);
            break;
        }
    #endif
    #if FEATURE_GMAC_RX_NUM_CHANNELS > 5U
        case 5U:
        {
            Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q5MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP1_Q5DDMACH(1U);
            break;
        }
    #endif
    #if FEATURE_GMAC_RX_NUM_CHANNELS > 6U
        case 6U:
        {
            Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q6MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP1_Q6DDMACH(1U);
            break;
        }
    #endif
    #if FEATURE_GMAC_RX_NUM_CHANNELS > 7U
        case 7U:
        {
            Base->MTL_RXQ_DMA_MAP1 |= GMAC_MTL_RXQ_DMA_MAP1_Q7MDMACH(0U) |
                                      GMAC_MTL_RXQ_DMA_MAP1_Q7DDMACH(1U);
            break;
        }
    #endif
        default:
                                ...
                                const Gmac_CtrlConfigType *Config)
{
    ...
    GMAC_DEV_ASSERT((Config->Gmac_pCtrlConfig->RxRingCount > 0U) && (Config->Gmac_pCtrlConfig->RxRingCount <= FEATURE_GMAC_RX_NUM_CHANNELS));
    GMAC_DEV_ASSERT((Config->Gmac_pCtrlConfig->TxRingCount > 0U) && (Config->Gmac_pCtrlConfig->TxRingCount <= FEATURE_GMAC_TX_NUM_CHANNELS));
...
#if (STD_ON == GMAC_IP_PPS_OUTPUT_SUPPORT)
        /* Enable Flexible PPS output mode at the beginning to prevent any pulses generated!
         * Note: In Flexible PPS mode, the pulses generated only a non-zero command set.
        */
...
#if defined(GMAC_IP_FRAME_PREEMPTION_ENABLE) && (STD_ON == GMAC_IP_FRAME_PREEMPTION_ENABLE)
        if (NULL_PTR != Config->Gmac_pCtrlFramePreemption)
        {
            Gmac_Ip_FramePreemptionInit(Base, Config->Gmac_pCtrlFramePreemption);
        }
#endif
    }
                                     ...
                                     uint16 * BuffId)
{
    ...
    Gmac_Ip_StatusType Status = GMAC_STATUS_SUCCESS;
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    uint16 NoOfDescriptors = 0U;
    uint16 CurrentDescriptorIndex = 0U;
#endif
    GMAC_DEV_ASSERT(Instance <  FEATURE_GMAC_NUM_INSTANCES);
    ...
    SchM_Enter_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_00();
    ...
    else
    {
        ...
        if (Buff->Length > Gmac_Ip_ScatterGatherLastTxBuffLength[Instance])
        {
            Status = GMAC_STATUS_TX_BUFF_OVERFLOW;
            Buff->Length = Gmac_Ip_ScatterGatherLastTxBuffLength[Instance];
        }
#else
        ...
        else
        {
            ...
            if (!Gmac_Ip_InstHasExternalTxBufferManagement[Instance])
            {
#endif
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
                /* Calculate the number of BD that are needed to transfer the frame */
                NoOfDescriptors = Buff->Length / (uint16)(Bd->Info1 & GMAC_INFO1_LENGTH_MASK);
                if ((Buff->Length % (uint16)(Bd->Info1 & GMAC_INFO1_LENGTH_MASK)) != 0U)
                {
                    NoOfDescriptors++;
                }
#endif
...
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
                /* Loop through the BD and check if they are all available. Later lock them, only if all are available. */
                while ((CurrentDescriptorIndex < NoOfDescriptors) && ((Gmac_Ip_PtrSizeType)Bd < (Gmac_Ip_PtrSizeType)&ListBd[ChBase->DMA_TXDESC_RING_LENGTH + 1UL]))
                {
                    /* When OWN bit is reset (0), application owns the Descriptor => no transfer is in progress and
                       When Locked bit is reset (0), BD is not reserved for other transfer */
                    if (((Bd->Des3 & GMAC_TDES3_OWN_MASK) == 0U) &&
                        ((Bd->Info1 & GMAC_INFO1_LOCKED_MASK) == 0U))
                    {
                        Bd++;
                    }
                    else /* Busy BD return GMAC_STATUS_TX_BUFF_BUSY */
                    {
                        Buff->Data = NULL_PTR;
                        Status = GMAC_STATUS_TX_BUFF_BUSY;
                        break;
                ...
                /* In case there are enough BDs to send the current frame, lock the BD*/
                if (Status == GMAC_STATUS_SUCCESS)
                {
                    /* Update the  Number of Descriptors locked(if Bd reached the end of the queue, there is enough space for the frame in the last BD) */
                    NoOfDescriptors = CurrentDescriptorIndex;

                    Bd =  Gmac_apxState[Instance]->TxCurrentDesc[Ring];
#endif  /* (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE) */
                    if (BuffId != NULL_PTR)
                    {
                        /* Calculate the index of the last BD locked. */
                        *BuffId = (uint16)((sint32)(Bd - ListBd));
                    }
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
                    for(CurrentDescriptorIndex = 0U; CurrentDescriptorIndex < NoOfDescriptors; CurrentDescriptorIndex++ )
                    {
#endif
                    /* Lock the BD */
                    Bd->Info1 |= GMAC_INFO1_LOCKED_MASK;
...
#endif /*(STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE) */
                    /* Set this variable to the first available Tx Descriptor. Check if Bd has reached the end of the queue or not.
                    If it did, Set the next available Tx Descriotor to the first Descriptor in Queue*/
                    Gmac_apxState[Instance]->TxCurrentDesc[Ring] = ((Gmac_Ip_PtrSizeType)Bd >= (Gmac_Ip_PtrSizeType)&ListBd[ChBase->DMA_TXDESC_RING_LENGTH + 1U] ) ?  ListBd : Bd;
    ...
    SchM_Exit_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_00();
    return Status;
}
                                                  ...
                                                  uint16 *buffId)
{
    ...
    if (FreeBuffers == NumBuffers)
    {
        *buffId = (uint16)((sint32)(Gmac_apxState[Instance]->TxCurrentDesc[ring] - ListBd));
        Gmac_apxState[Instance]->TxCurrentDesc[ring] = Bd;
    }
                                     ...
                                     const Gmac_Ip_TxOptionsType * Options)
{
    ...
    Gmac_Ip_BufferDescriptorType *Bd;
    const Gmac_Ip_BufferDescriptorType *ListBd;
    Gmac_Ip_StatusType Status = GMAC_STATUS_SUCCESS;
#if (STD_ON == GMAC_HAS_CACHE_MANAGEMENT)
    Std_ReturnType CacheStatus = E_NOT_OK;
#endif
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    Gmac_Ip_BufferDescriptorType *FirstBdUsed;
    uint16 BdBufferLength = 0U;
    uint16 FrameLength = 0U;
    Gmac_Ip_PtrSizeType LastBdInQueue;
    Gmac_Ip_BufferDescriptorType *LastBdUsed;
    ...
    SchM_Enter_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_01();

    Bd = (Gmac_Ip_BufferDescriptorType *)((Gmac_Ip_PtrSizeType)Base->DMA_TXDESC_TAIL_POINTER);
    ListBd = (const Gmac_Ip_BufferDescriptorType *)((Gmac_Ip_PtrSizeType)Base->DMA_TXDESC_LIST_ADDRESS);
    ...
    {
        /* Set the first descriptor with the pointer to the data buffer
            For S/C there is no need to complete all the intermediary addresses */
        Bd->Des0 = (uint32)(Gmac_Ip_PtrSizeType)Buff->Data;

#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
        /* Calculate the number of BD that are needed to transfer the frame */
        BdBufferLength = (uint16)(Bd->Info1 & GMAC_INFO1_LENGTH_MASK);
        /* Mark the First Descriptor used for the current transfer */
        FirstBdUsed = Bd;
        /* Save the Frame Length */
        FrameLength = Buff->Length;
        /* Get the address of the last descriptor allocated for the current Ring*/
        LastBdInQueue = (Gmac_Ip_PtrSizeType)&ListBd[Base->DMA_TXDESC_RING_LENGTH];
        /* This loop will ensure that all the descriptors (needed to transfer the current frame)
           are filled with Length information,
           The Info0 pointer holds a copy of the address of data frame that will be used in case of write back.*/
        while (FrameLength > 0U)
        {
            Bd->Info0  = (uint32)(Gmac_Ip_PtrSizeType)&Buff->Data[Buff->Length - FrameLength];
            /* Check the Descritor is not the last */
            if (((Gmac_Ip_PtrSizeType)Bd < LastBdInQueue) && ((uint32)FrameLength > BdBufferLength))
            {
                /* Set the buffer length (different of Frame length)*/
                Bd->Des2 = BdBufferLength;
                FrameLength -= BdBufferLength ;
                Bd++;
            }
            /* Check the Descriptor is the last or there is left less than the allocated length for the current descriptor */
            else if (((Gmac_Ip_PtrSizeType)Bd == LastBdInQueue) || ((uint32)FrameLength <= BdBufferLength))
            {
                Bd->Des2 = FrameLength;
                break;
            }
        ...
        /* Save the address of Last descriptor used in the current transfer*/
        LastBdUsed = Bd;
        /* Set the IOC (Interrupt on completion) and TTSE(Transmit Timestamp Enable) ONLY for the Last Descritor.
        The Write Back format is applicable only for the Last Descriptor, thus the Timestamp info will be read from Last Descriptor */
        Bd->Des2 |= GMAC_TDES2_IOC_MASK | GMAC_TDES2_TTSE_MASK;
#else
        ...
        /* To discuss the approach for S/C - Is it faster to write the DES0-2 for all the BD involved in transfer and then wait for all to update, or to have a barrier after each writing*/
...
#else
        /* Set the FD First Descriptor bit */
        FirstBdUsed->Des3 |= GMAC_TDES3_FD_MASK;
        /* Set the LD Last Descriptor bit */
        LastBdUsed->Des3 |= GMAC_TDES3_LD_MASK;

        if (Options != NULL_PTR)
        {
            /*These fields are valid only when the first descriptor bit is set */
            FirstBdUsed->Des3 |= GMAC_TDES3_CPC(Options->CrcPadIns) |
                                 GMAC_TDES3_CIC(Options->ChecksumIns);

            if (Options->NoInt)
            {
                LastBdUsed->Des2 &= ~GMAC_TDES2_IOC_MASK;
            }
        }

        for (Bd = FirstBdUsed; Bd <= LastBdUsed; Bd++)
        {
            /*This field is equal to the length of the packet to be transmitted in bytes. When the TSE bit is not set, this field is equal
                to the total length of the packet to be transmitted */
            Bd->Des3 |= (uint32)Buff->Length;
            /* GMAC_TDES3_OWN_MASK should be written at the last step in the sequence of writing BD. */
            Bd->Des3 |= GMAC_TDES3_OWN_MASK;
        }
    ...
    SchM_Exit_Eth_43_GMAC_ETH_EXCLUSIVE_AREA_01();
...
#if (STD_ON == GMAC_IP_RX_HEADER_SPLIT)
        CacheStatus =  Cache_Ip_InvalidateByAddr(CacheType, CACHE_IP_DATA, (uint32)Bd->Info0, Bd->Des2 & GMAC_RDES2_HL_MASK);
        CacheStatus |=  Cache_Ip_InvalidateByAddr(CacheType, CACHE_IP_DATA, (uint32)Bd->Padding1, ((Bd->Des3 & GMAC_RDES3_PL_MASK) - (Bd->Des2 & GMAC_RDES2_HL_MASK)));
#else
...
#endif
        {
            Status = Gmac_Ip_ProcessRxDesc(Instance, Ring, Bd, Buff, Info);
        }
    }
                           ...
                           const Gmac_Ip_BufferType * Buff)
{
    ...
    /* When receive split header functions is enable -> Buffer2 or Payload data address buffer should be marked as valid. */
    Bd->Des3 = GMAC_RDES3_OWN_MASK | GMAC_RDES3_INTE_MASK | GMAC_RDES3_BUF1V_MASK | GMAC_RDES3_BUF2V_MASK;
    Bd->Padding1 = (Gmac_Ip_PtrSizeType)Buff[1U].Data;
#else
    Bd->Des3 = GMAC_RDES3_OWN_MASK | GMAC_RDES3_INTE_MASK | GMAC_RDES3_BUF1V_MASK;
#endif
...
#if (GMAC_IP_SCATTER_GATHER_ENABLE == STD_ON)
                                             ...
                                             Gmac_Ip_TxInfoType * Info)
{
    ...
    Gmac_Ip_BufferDescriptorType *Bd;
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    Gmac_Ip_BufferDescriptorType *StartBd;
#endif
...
# if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
            ...
            while ((Bd->Des3 & GMAC_TDES3_LD_MASK) == 0U)
            {
                if (((Bd->Des3 & GMAC_TDES3_OWN_MASK) != 0U))
                {
                    Status = GMAC_STATUS_BUSY;
            ...
            {
                Bd = StartBd;   /* Prevent endless loop */
#endif
                Status = Gmac_Ip_ProcessTxDesc(Instance, Ring, Bd, Info);
                break;
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
            }
#endif
...
#if (STD_ON == GMAC_IP_SCATTER_GATHER_ENABLE)
    /* When scatter gather feature is enabled, there is only one request registered in the HLD for each frame regardless of the number of descritors used in transfer
    This line does not work with SendMultiBuffer flow */
    Gmac_Ip_u16NumberOfClearedBd[Instance][Ring] = 1U;
#endif
                                       ...
                                       const Gmac_Ip_SysTimeConfigType *SysTimeConfig)
{
    ...
    Base->MAC_TIMESTAMP_CONTROL |= GMAC_MAC_TIMESTAMP_CONTROL_TSINIT_MASK | GMAC_MAC_TIMESTAMP_CONTROL_PTGE_MASK;
...
void Gmac_Ip_GetSysTime(uint8 Instance, Gmac_Ip_TimestampType * Timestamp)
{
    ...
    } while (Base->MAC_SYSTEM_TIME_SECONDS != Timestamp->seconds);
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_SetPpsSignalMode
 * Description   : Switching between Flexible PPS mode or Fixed PPS mode
 *
 *END**************************************************************************/
void Gmac_Ip_SetPpsSignalMode(uint8 Instance, boolean SignalMode)
{
    ...
    if (SignalMode)
    {
        /* Switch to flexible PPS output mode. */
        Base->MAC_PPS_CONTROL |= GMAC_MAC_PPS_CONTROL_PPSEN0(1U);
    }
    else
    {
        /* Switch to Fixed PPS output mode. */
...
void Gmac_Ip_PPSOutputSignalInit(uint8 Instance, uint32 ModuleClk, uint8 PPSOutputsNum, const Gmac_Ip_FlexiblePPSOutput PPSOutputConfig[])
{
    ...
    GMAC_DEV_ASSERT(Instance <  FEATURE_GMAC_NUM_INSTANCES);
    GMAC_DEV_ASSERT(ModuleClk != 0U);
    GMAC_DEV_ASSERT(PPSOutputConfig != NULL_PTR);
    GMAC_DEV_ASSERT(PPSOutputsNum != 0U);

    Base = Gmac_apxBases[Instance];

    for (PPSOutputCount = 0U; PPSOutputCount < PPSOutputsNum; PPSOutputCount++)
    {
        GMAC_DEV_ASSERT(PPSOutputConfig[PPSOutputCount].PPSIndex < FEATURE_GMAC_PPS_NUM_CHANNELS);
        PPSInterval = (uint32)(ModuleClk / PPSOutputConfig[PPSOutputCount].PPSFrequency);
        PPSWidth = (uint32)((uint64)PPSInterval * PPSOutputConfig[PPSOutputCount].PPSDutyCycle / 100U);
        /* In order to generate the valid pulse: */
        GMAC_DEV_ASSERT(PPSInterval >  PPSWidth);
        GMAC_DEV_ASSERT(PPSInterval >  0);
        GMAC_DEV_ASSERT(PPSWidth >  0);
        Base->MAC_PPS_CONTROL &= ~(GMAC_MAC_PPS_CONTROL_TRGTMODSEL0_MASK << (8U * (PPSOutputConfig[PPSOutputCount].PPSIndex)));
        Base->MAC_PPS_CONTROL |= ((GMAC_MAC_PPS_CONTROL_TRGTMODSEL0(PPSOutputConfig[PPSOutputCount].PPSModeSelect)) << (8U * (PPSOutputConfig[PPSOutputCount].PPSIndex)));

        MacPPSIntervalRegAddr = (uint32*)((Gmac_Ip_PtrSizeType)(&Base->MAC_PPS0_INTERVAL) + GMAC_MAC_PPS_CONTROL_REG_OFFSET(PPSOutputConfig[PPSOutputCount].PPSIndex));
        MacPPSWidthRegAddr = (uint32*)((Gmac_Ip_PtrSizeType)(&Base->MAC_PPS0_WIDTH) + GMAC_MAC_PPS_CONTROL_REG_OFFSET(PPSOutputConfig[PPSOutputCount].PPSIndex));
        *MacPPSIntervalRegAddr = GMAC_MAC_PPS0_INTERVAL_PPSINT0(PPSInterval - 1U);
        *MacPPSWidthRegAddr = GMAC_MAC_PPS0_WIDTH_PPSWIDTH0(PPSWidth - 1U);
    }
}

/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_GeneratePulsePerSecondOutput
 * Description   : Perform generating Flexible pulse(s) on Pulse Per Second
 *
 *END**************************************************************************/
Gmac_Ip_StatusType Gmac_Ip_GeneratePulsePerSecondOutput(uint8 Instance, uint8 PPSOutputIdx, Gmac_Ip_PPSTargetTime * PPSTargetTime, Gmac_Ip_PPSOutputCmd PPSOutputCmd)
{
    ...
    GMAC_DEV_ASSERT(Instance <  FEATURE_GMAC_NUM_INSTANCES);
    GMAC_DEV_ASSERT(PPSOutputIdx <  FEATURE_GMAC_PPS_NUM_CHANNELS);
    GMAC_DEV_ASSERT(PPSTargetTime != NULL_PTR);
    Base = Gmac_apxBases[Instance];

    /* Work only with Flexible PPS mode selected */
    if (0U != (Base->MAC_PPS_CONTROL & GMAC_MAC_PPS_CONTROL_PPSEN0_MASK))
    {
        ...
        *MacPPSNanosecondRegAddr = GMAC_MAC_PPS0_TARGET_TIME_NANOSECONDS_TTSL0(PPSTargetTime->nanoseconds);
        ...
        Base->MAC_PPS_CONTROL &= ~(GMAC_MAC_PPS_CONTROL_PPSCTRL_PPSCMD_MASK << (8U * PPSOutputIdx));
    ...
    {
        /* The given PPSOutputCmd has no meaning when Fixed PPS mode selected, just return ERROR */
        Status = GMAC_STATUS_ERROR;
    }
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Gmac_Ip_GenerateFixedPulsePerSecondOutput
 * Description   : Perform generating Fixed PPS pulses
 *
 *END**************************************************************************/
Gmac_Ip_StatusType Gmac_Ip_GenerateFixedPulsePerSecondOutput(uint8 Instance, Gmac_Ip_FixedPPSClockType FixedClock)
{
    GMAC_Type *Base;
    Gmac_Ip_StatusType Status = GMAC_STATUS_SUCCESS;

    GMAC_DEV_ASSERT(Instance <  FEATURE_GMAC_NUM_INSTANCES);
    GMAC_DEV_ASSERT((uint8)FixedClock <= (uint8)GMAC_PPS_16384HZ);
    Base = Gmac_apxBases[Instance];

    /* Work only with Fixed PPS mode selected */
    if (0U == (Base->MAC_PPS_CONTROL & GMAC_MAC_PPS_CONTROL_PPSEN0_MASK))
    {
        Base->MAC_PPS_CONTROL &= ~GMAC_MAC_PPS_CONTROL_PPSCTRL_PPSCMD_MASK;
        Base->MAC_PPS_CONTROL |= GMAC_MAC_PPS_CONTROL_PPSCTRL_PPSCMD(FixedClock);
    }
    else
    {
        /* The given command has no meaning when Flexible PPS mode selected, just return ERROR */
        Status = GMAC_STATUS_ERROR;
    }
