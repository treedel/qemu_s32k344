...
static uint64_t s32k389_boot_status_read(void *opaque, hwaddr offset, unsigned size) {
    switch (offset) {
...
static void s32k389_boot_status_write(void *opaque, hwaddr offset, uint64_t value, unsigned size) {
    qemu_log_mask(LOG_UNIMP,
                  "s32k389.boot-status: unimplemented write "
                  "(size %u, offset 0x%" HWADDR_PRIx ", value 0x%" PRIx64 ")\n",
                  size, offset, value);
}

static const MemoryRegionOps s32k389_boot_status_ops = {
    .read = s32k389_boot_status_read,
    .write = s32k389_boot_status_write,
...
static bool s32k389_mc_me_is_cofb_status(hwaddr offset)
{
...
static uint64_t s32k389_mc_me_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    if (size != 4 || offset + size > S32K3_MC_ME_SIZE) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "s32k389.mc_me: invalid read size %u "
                      "@0x%" HWADDR_PRIx "\n", size, offset);
        return 0;
    }

    if (s32k389_mc_me_is_cofb_status(offset)) {
        return UINT32_MAX;
    }
...
static void s32k389_mc_me_write(void *opaque, hwaddr offset,
                                uint64_t value, unsigned size)
{
    ...
    if (size != 4 || offset + size > S32K3_MC_ME_SIZE) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "s32k389.mc_me: invalid write size %u "
...
static void s32k389_init_tcm(S32K389State *s, MemoryRegion *system_memory)
{
    static const hwaddr itcm_backdoor_bases[S32K389_TCM_COUNT] = {
        INT_ITCM0_BACKDOOR_BASE,
        INT_ITCM1_BACKDOOR_BASE,
        INT_ITCM2_BACKDOOR_BASE,
        INT_ITCM3_BACKDOOR_BASE,
    };
    static const hwaddr dtcm_backdoor_bases[S32K389_TCM_COUNT] = {
        INT_DTCM0_BACKDOOR_BASE,
        INT_DTCM1_BACKDOOR_BASE,
        INT_DTCM2_BACKDOOR_BASE,
        INT_DTCM3_BACKDOOR_BASE,
    };

    for (int i = 0; i < S32K389_TCM_COUNT; i++) {
        char name[32];

        snprintf(name, sizeof(name), "S32K389.itcm%d", i);
        memory_region_init_ram(&s->itcm[i], NULL, name, INT_ITCM_SIZE,
                               &error_fatal);

        snprintf(name, sizeof(name), "S32K389.dtcm%d", i);
        memory_region_init_ram(&s->dtcm[i], NULL, name, INT_DTCM_SIZE,
                               &error_fatal);

        snprintf(name, sizeof(name), "S32K389.itcm%d.backdoor", i);
        memory_region_init_alias(&s->itcm_backdoor[i], NULL, name,
                                 &s->itcm[i], 0, INT_ITCM_SIZE);
        memory_region_add_subregion(system_memory, itcm_backdoor_bases[i],
                                    &s->itcm_backdoor[i]);

        snprintf(name, sizeof(name), "S32K389.dtcm%d.backdoor", i);
        memory_region_init_alias(&s->dtcm_backdoor[i], NULL, name,
                                 &s->dtcm[i], 0, INT_DTCM_SIZE);
        memory_region_add_subregion(system_memory, dtcm_backdoor_bases[i],
                                    &s->dtcm_backdoor[i]);
    }

    memory_region_add_subregion(system_memory, INT_ITCM_BASE, &s->itcm[0]);
    memory_region_add_subregion(system_memory, INT_DTCM_BASE, &s->dtcm[0]);
}
...
static void s32k389_init(MachineState* machine) {
    ...
    s32k389_init_tcm(s, system_memory);

    // Flash
    memory_region_init_rom(&s->C0flash, NULL, "S32K389.C0flash", INT_CODE_FLASH0_SIZE, &error_fatal);
    memory_region_add_subregion(system_memory, INT_CODE_FLASH0_BASE, &s->C0flash);
    memory_region_init_rom(&s->C1flash, NULL, "S32K389.C1flash", INT_CODE_FLASH1_SIZE, &error_fatal);
    memory_region_add_subregion(system_memory, INT_CODE_FLASH1_BASE, &s->C1flash);
    memory_region_init_rom(&s->C2flash, NULL, "S32K389.C2flash", INT_CODE_FLASH2_SIZE, &error_fatal);
    memory_region_add_subregion(system_memory, INT_CODE_FLASH2_BASE, &s->C2flash);
    memory_region_init_rom(&s->C3flash, NULL, "S32K389.C3flash", INT_CODE_FLASH3_SIZE, &error_fatal);
    ...
    memory_region_add_subregion_overlap(system_memory, S32K3_MC_ME_BASE,
                                        &s->mc_me, 1);
    memory_region_init_io(&s->boot_status, NULL, &s32k389_boot_status_ops, s, "s32k389.boot-status", S32K3_BOOT_STATUS_SIZE);
